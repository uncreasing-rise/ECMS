...
export class ExamsService {
  ...
  async createQuestion(createdBy: string, dto: QuestionDraft) {
    if (!dto.question_format || !dto.content) {
      throw new AppException({
        ...
        errorKey: 'exam.bad_request',
        message: 'question_format và content là bắt buộc',
      });
    }
  ...
  async importQuestions(createdBy: string, dto: ImportQuestionsDto) {
    ...
    const normalizedQuestions = questions.filter(
      (
        item,
      ): item is QuestionDraft & { question_format: string; content: string } =>
    ...
    if (normalizedQuestions.length === 0) {
      return {
        imported: 0,
        errors: [
          'Thiếu question_format hoặc content cho toàn bộ câu hỏi import',
        ],
      };
    }
  ...
  async submitQuestionForReview(questionId: string, userId: string) {
    ...
    if (question.created_by !== userId) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Chỉ chủ sở hữu mới được gửi review',
      });
    }
  ...
  async createManualExam(createdBy: string, dto: ManualExamDto) {
    if (
      ...
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'title và questions là bắt buộc',
      });
    }
    ...
    if (!dto.duration_minutes || dto.duration_minutes <= 0) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'duration_minutes là bắt buộc và phải > 0',
      });
    }
    ...
    await this.prisma.exam_questions.createMany({
      data: dto.questions.map(
        (item: ManualExamQuestionItem, index: number) => ({
          id: randomUUID(),
          exam_id: exam.id,
  ...
  async createAutoExam(createdBy: string, dto: AutoExamDto) {
    if (!dto.title || !Array.isArray(dto.matrix) || dto.matrix.length === 0) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'title và matrix là bắt buộc',
      });
    }
    ...
    if (uniqueQuestionIds.length === 0) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Không chọn được câu hỏi phù hợp từ ma trận',
      });
    }
  ...
  async assignExamToClasses(
    ...
    if (activeEnrollments.length > 0) {
      ...
      for (const enrollment of activeEnrollments) {
        const classNames =
          classesByStudent.get(enrollment.student_id) ?? new Set<string>();
      ...
      await this.notifications.createBulk(
        Array.from(classesByStudent.entries()).map(
          ([studentId, classNames]) => ({
            user_id: studentId,
  ...
  async startOrResumeSession(
    ...
    if (exam.available_from && now < exam.available_from) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Chưa đến thời gian mở bài thi',
      });
    }
    if (exam.available_until && now > exam.available_until) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Đã hết cửa sổ thời gian bắt đầu bài thi',
      });
    }
    ...
    if (!assignedClass) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Bạn không thuộc lớp được giao đề thi này',
      });
    }
  ...
  async autosaveAnswer(
    ...
    if (!session) {
      throw new AppException({
        code: AppErrorCode.NOT_FOUND,
        errorKey: 'exam.not_found',
        message: 'Không tìm thấy phiên thi',
      });
    }
    ...
    if (session.student_id !== studentId) {
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Bạn không có quyền với phiên thi này',
      });
    }
    ...
    if (session.expires_at.getTime() < Date.now()) {
      ...
      throw new AppException({
        code: AppErrorCode.BAD_REQUEST,
        errorKey: 'exam.bad_request',
        message: 'Phiên thi đã hết giờ và được nộp tự động',
      });
    }
